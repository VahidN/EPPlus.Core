...
namespace EPPlus.Core.Tests.SampleApp
{
    ...
    {
        [TestMethod]
        public void RunSample10()
        {
            //Create a Sample10 directory...
            var dir = Path.Combine("bin", "Sample10");
            ...
            FileInfo templateFile = new FileInfo(Path.Combine(dir, "Template.xlsx"));
            if(templateFile.Exists)
            {
                templateFile.Delete();
            }

            FileInfo answerFile = new FileInfo(Path.Combine(dir, "Answers.xlsx"));
            if(answerFile.Exists)
            {
                answerFile.Delete();
            }

            FileInfo JKAnswerFile = new FileInfo(Path.Combine(dir, "JKAnswers.xlsx"));
            ...
            foreach (var cell in wsAnswers.Cells["C7,C9,C11"])
            {
                ...
                if (cell.Value.ToString().Equals(wsUser.Cells[cell.Address].Value.ToString(), StringComparisonEx.InvariantCultureIgnoreCase)) //Correct Answer?
                {
